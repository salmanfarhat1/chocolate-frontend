...
async function getVariants(id: string): Promise<Variant[]> {
  try {
    const res = await fetch(`${API}/chocolates/${id}/variants`, {
      cache: "no-store",
    });
    ...
    return (
      <div className="min-h-screen bg-[#fdfaf5]">
        ...
        <div className="flex flex-col items-center justify-center py-32">
          ...
          <h1 className="text-2xl font-semibold text-[#5a2a27] mb-2">
            Product not found
          </h1>
          <a href="/chocolates" className="text-[#8b7355] underline mt-4">
            ← Back to collection
          </a>
        </div>
        <Footer />
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
              ...
              <div className="flex gap-3 mt-4 flex-wrap">
                {photos.map((p, i) => (
                  <img
                    ...
                    src={`${API}${p}`}
                    alt={`photo ${i + 1}`}
                    className="w-20 h-20 object-cover rounded-lg border-2 border-transparent hover:border-[#5a2a27]"
                  />
                ))}
              </div>
            )}
          ...
          <div>
            ...
            <h1 className="text-4xl font-light text-[#5a2a27] mb-6">
              {chocolate.name}
            </h1>

            {chocolate.ingredients && (
              <div className="mb-8">
                <div className="text-xs uppercase tracking-widest text-[#8b7355] mb-2">
                  Ingredients
                </div>
                <p className="text-[#7a5a53] leading-relaxed">
                  {chocolate.ingredients}
                </p>
              </div>
            )}
